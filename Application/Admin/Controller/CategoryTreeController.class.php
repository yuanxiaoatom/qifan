...
class CategoryTreeController extends AdminController {
	public function index(){
		$tree = D('CategoryTree')->getTree(0,'id,name,title,sort,pid,allow_publish,status');
	...
	public function remove(){
		...
		$child = M('CategoryTree')->where(array('pid'=>$cate_id))->field('id')->select();
		if(!empty($child)){
		...
		//删除该分类信息
		$res = M('CategoryTree')->delete($cate_id);
		if($res !== false){
			//记录行为
			action_log('update_category', 'categoryTree', $cate_id, UID);
			$this->success('删除分类成功！');
		}else{
			$this->error('删除分类失败！');
		}
	...
	public function operate($type = 'move'){
		...
		$list = M('CategoryTree')->where($map)->field('id,pid,title')->select();
		...
		if(strcmp($type, 'move') == 0){
			...
			$pid = M('CategoryTree')->getFieldById($from, 'pid');
	...
	public function move(){
		$to = I('post.to');
		$from = I('post.from');
		$res = M('CategoryTree')->where(array('id'=>$from))->setField('pid', $to);
		if($res !== false){
